...
I want emacs to use the system default monospace font (set via fontconfig).
Emacs doesn't honor fontconfig's generic-alias preferences for the default
face, so ="monospace"= resolves to whatever emacs picks on its own rather than
the configured font. Every other app respects it fine.

So I ask =fc-match= for the concrete family that =monospace= resolves to and
hand emacs that exact name, keeping fontconfig the single source of truth. If
=fc-match= isn't on PATH, fall back to =monospace-12=.
...
(defvar my-monospace-font
  (if (executable-find "fc-match")
      (car (split-string
            ...
            ","))
    "monospace")
  "Concrete monospace family resolved from fontconfig at startup.")

(let ((font (concat my-monospace-font "-12")))
...
Custom mode I wrote for the daml smart contract language, which
is based on Haskell.
#+BEGIN_SRC emacs-lisp
(sup '(daml-ts-mode
       :host github
